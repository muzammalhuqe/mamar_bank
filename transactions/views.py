...
# def send_trasaction_email(user, amount, subject, template):
...
def send_receiver_email(user, amount, subject, template):
        message = render_to_string(template, {
            'to_user' : user,
            'amount' : amount,
        })
        ...
        if form.is_valid():
            ...
            try:
                ...
                if current_user.balance >= amount:
                    ...
                    print(to_user.user.email)

                    send_transaction_email(self.request.user, amount, "Transfer Balance", "transactions/transfer_email.html")
                    send_receiver_email(to_user, amount, "Received Balance", "transactions/receiver_email.html")
                else:
                     messages.error(request, 'Insufficient balance in the senders account.')
            except UserBankAccount.DoesNotExist:
                 messages.error(request, 'Receiver account not found. Please check the account number.')

            return render(request, self.template_name, {'form': form, 'title' : 'Tranfer Money'})
